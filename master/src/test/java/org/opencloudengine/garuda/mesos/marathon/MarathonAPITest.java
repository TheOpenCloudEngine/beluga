...
public class MarathonAPITest {
    ...
    public void getApp() {
        ...
        Map<String, App> getApp = client.target(target).path(path).request().accept(MediaType.APPLICATION_JSON_TYPE).get(new GenericType<Map<String, App>>(){});
        App app = getApp.get("app");
        System.out.println(app);
        System.out.println("id : " + app.getId());
        System.out.println("cmd : " + app.getCmd());
    ...
    public void deployApp() {
        ...
        app.setMem(16.0f);
        app.setInstances(1);
        logger.debug("data : {}", Entity.json(app).toString());

        App appResponse = client.target(target).path(path).request(MediaType.APPLICATION_JSON_TYPE).accept(MediaType.APPLICATION_JSON_TYPE).post(Entity.json(app), App.class);
